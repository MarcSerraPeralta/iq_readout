...
def test_ThreeStateLinearClassifier2D():
    ...
    cla = ThreeStateLinearClassifier2D().fit(shots_0, shots_1, shots_2)
...
def test_load():
    cla = ThreeStateLinearClassifier2D().load(PARAMS)
    assert cla.params() == PARAMS

    with pytest.raises(ValueError) as e_info:
        cla = ThreeStateLinearClassifier2D().load({})
...
def test_n_bins():
    ...
    with pytest.raises(ValueError) as e_info:
        cla = ThreeStateLinearClassifier2D().fit(shots_0, shots_1, shots_2, n_bins=2)
    return
